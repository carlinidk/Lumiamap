...
let currentCodeMode = 'canvas';
let codePreviewCanvas = null;
...
function renderVideoSources() {
    ...
    videoSources.forEach(source => {
        ...
        div.innerHTML = `
            ...
            <i data-lucide="${icon}" class="w-4 h-4 ${source.isCode ? 'text-purple-400' : 'text-green-400'}"></i>
        `;
...
function draw(ctx, width, height, time) {
    ...
    for (let x = 0; x < width; x += 2) {
        const y = height / 2 + Math.sin((x / width) * Math.PI * 4 + time) * height / 4;
...
window.closeCodeEditorModal = closeCodeEditorModal;
...
document.querySelectorAll('.tab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        currentCodeMode = btn.dataset.mode;
    });
});
...
function loadTemplate(templateId) {
    ...
    if (template) {
        ...
        currentCodeMode = template.mode;
        
        // Update active tab
        document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active'));
        document.querySelector(`[data-mode="${template.mode}"]`).classList.add('active');
    }
}
...
previewCodeBtn.addEventListener('click', () => {
    ...
    try {
        createCodeSource(name, code, currentCodeMode, true);
        alert('Preview created! Check your sources list.');
    } catch (e) {
        alert('Error in code: ' + e.message);
    }
});

// Save snippet as source
saveSnippetBtn.addEventListener('click', () => {
    ...
    try {
        createCodeSource(name, code, currentCodeMode, false);
...
// Create a code-based media source
function createCodeSource(name, code, mode, isPreview) {
    ...
    if (mode === 'canvas') {
        // Canvas 2D mode
        const drawFn = new Function('ctx', 'width', 'height', 'time', code + '\ndraw(ctx, width, height, time);');
        ...
        if (!gl.getShaderParameter(fragmentShader, gl.COMPILE_STATUS)) {
            throw new Error('Shader compile error: ' + gl.getShaderInfoLog(fragmentShader));
        }
        ...
        // p5.js mode
        alert('p5.js mode coming soon! Use Canvas 2D for now.');
        return;
    }
